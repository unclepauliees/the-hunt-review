...
export function NarrativeAct({ act }: { act: Act }) {
  ...
  return (
    <section ref={root} id={act.id} data-act-index={act.index - 1} className={`narrative-act ${!act.image ? "text-only" : ""} ${act.id === "structure" ? "structure-act" : ""} ${act.id === "premise" ? "premise-act" : ""} ${act.id === "corridor" ? "corridor-act" : ""} ${act.id === "arrival" ? "arrival-act" : ""} ${act.id === "dresscode" ? "dresscode-act" : ""} ${act.id === "atmosphere" ? "atmosphere-act" : ""} ${act.id === "revelation" ? "revelation-act" : ""}`}>
